(function(window, utils) {
    ...
    function createDesign(categoryHistory) {
        var categoryLength = categoryHistory.length;
        var count = 0;
        if(categoryHistory.length < 4){
            categoryHistory.push(
                {secondarycategory: "POWER TOOLS", primarycategory: "TOOLS"}, 
                {secondarycategory: "BUILDING & CONSTRUCTION", primarycategory: "BUILDING & HARDWARE"}, 
                {secondarycategory: "PLANT POTS, PLANTERS & BASKETS", primarycategory: "GARDEN"},
        				{secondarycategory: "BBQS", primarycategory: "OUTDOOR LIVING"}
            );
            for(var i = 0; i<categoryLength; i=i+1){
                if(categoryHistory[i].secondarycategory == 'POWER TOOLS')	{
                  categoryHistory.splice(categoryLength-count, 1);
                  count++;
                }
                else if(categoryHistory[i].secondarycategory == 'BUILDING & CONSTRUCTION')	{
                  categoryHistory.splice((categoryLength+1)-count, 1);
                  count++;
                }
                else if(categoryHistory[i].secondarycategory == 'PLANT POTS, PLANTERS & BASKETS')	{
                  categoryHistory.splice((categoryLength+2)-count, 1);
                  count++;
                }
                else if(categoryHistory[i].secondarycategory == 'BBQS')	{
                  categoryHistory.splice((categoryLength+3)-count, 1);
                  count++;
                }
            }
        }
        ...
        categoryHistory.slice(0, 4).forEach(function(categoryName) {
          	if(categoryName.secondarycategory.length > 23) flag = true;
        });
        ...
        categoryHistory.slice(0, 4).forEach(function(categoryName) {
            ...
            } else if((categoryName.secondarycategory) == 'APARTMENT KITCHEN' || (categoryName.secondarycategory) == 'CULINARY DELIGHT KITCHEN' || (categoryName.secondarycategory) == 'THE CLASSIC KITCHEN' ||(categoryName.secondarycategory) == 'URBAN COTTAGE KITCHEN'){
                categoryName.secondarycategory = 'KITCHEN INSPIRATION';
            }
            ...
            svgIcon.className = 'svg_icon';
            svgIcon.innerHTML = '<svg class="shop_button_chevron"><use xlink:href="#icon-chevron-guillemet"></use></svg>';
            link.appendChild(svgIcon);
            
            if(flag && !isMobile()){
              	product.classList.add("product_abs");
              	paragraph.classList.add("para_abs");
              	svgIcon.classList.add("svg_abs");
            }
            
            if(flag && categoryName.secondarycategory.length < 25){
                if(categoryName.secondarycategory.length < 20){
                    var br = document.createElement('span');
                    br.innerHTML = '<br>';
                    paragraph.firstChild.appendChild(br);
                }
              	paragraph.classList.add("short_desc");
            }
